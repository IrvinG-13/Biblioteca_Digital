...
class SolicitudController
{
    ...
    private function verificarSesionEstudiante(): void
    {
        $this->iniciarSesion();
    ...
    private function obtenerEstudianteSesion(): array
    {
        ...
        $estudiante =
            $this->estudianteModelo->obtenerPorUsuarioId(
                $usuarioId
            );

        if ($estudiante === null) {
            header(
                'Location: catalogo.php?error=perfil_estudiante'
            );
            exit;
        }
    ...
    {
        $estudiante = $this->obtenerEstudianteSesion();

        return [
            'estudiante' => $estudiante,
            'categorias' =>
                $this->solicitudModelo->obtenerCategorias()
        ];
    }
    ...
    public function guardar(): void
    {
        ...
        /*
         * También acepta "area" como respaldo
         * por si el navegador conserva un formulario antiguo.
         */
        ...
        if (
            ...
            header(
                'Location: solicitar_libro.php?error=titulo'
            );
            exit;
        }
        ...
        if (
            ...
            header(
                'Location: solicitar_libro.php?error=categoria'
            );
            exit;
        }
        ...
        if (strlen($comentario) > 1000) {
            header(
                'Location: solicitar_libro.php?error=comentario'
            );
            exit;
        }
        ...
        if (
            ...
            header(
                'Location: solicitar_libro.php?error=duplicada'
            );
            exit;
        }
        ...
        try {
            ...
            header(
                'Location: mis_solicitudes.php?exito=1'
            );
            exit;
        } catch (Throwable $e) {
            header(
                'Location: solicitar_libro.php?error=guardar'
            );
            exit;
        }
    }
    ...
    public function listarMisSolicitudes(): array
    {
        ...
        $solicitudes =
            $this->solicitudModelo->obtenerPorEstudiante(
                (int)$estudiante['id']
            );
    ...
    public function listarAdmin(): array
    {
        ...
        $total =
            $this->solicitudModelo->contarTotal(
                $categoria,
                $estado
            );
        ...
        $solicitudes =
            $this->solicitudModelo->obtenerTodas(
                $categoria,
                $estado,
                $porPagina,
                $offset
    ...
    public function cambiarEstado(): void
    {
        ...
        $observacion = Sanitizer::limpiarTexto(
            $_POST['observacion_admin'] ?? ''
        );

        $vistaActual = Sanitizer::limpiarTexto(
            $_POST['vista_actual'] ?? 'pendiente'
        );
        ...
        if ($categoriaActual !== '') {
            $parametrosRegreso['categoria'] =
                $categoriaActual;
        }

        if ($id <= 0) {
            $parametrosRegreso['error'] =
                'no_encontrada';

            header(
                'Location: solicitudes_admin.php?'
                . http_build_query($parametrosRegreso)
            );
            exit;
        }
        ...
        $solicitud =
            $this->solicitudModelo->obtenerPorId($id);
        ...
        try {
            $actualizada =
                $this->solicitudModelo->cambiarEstado(
                    $id,
                    $estado,
                    $usuarioGestorId,
                    $observacionFinal
